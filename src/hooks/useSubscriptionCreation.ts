...
import { SubscriptionFormData } from "../components/subscriptions/types";
import { generatePathSegment, generateSubdomain } from "../components/subscriptions/utils";
...
export function useSubscriptionCreation(isRTL: boolean, onSuccess: () => void) {
  ...
  const createSubscription = async (data: SubscriptionFormData) => {
    ...
    try {
      ...
      // Create the company first with is_subscription_company set to true
      ...
      // Generate unique path segment and subdomain
      ...
      // Update admin user profile with subscription_id and company_id
