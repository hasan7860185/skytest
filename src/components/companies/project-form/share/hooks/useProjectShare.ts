...
export const useProjectShare = ({ project, selectedFields, isRTL }: UseProjectShareProps) => {
  ...
  const handleWhatsAppShare = async () => {
    ...
    setIsLoading(true);
    try {
      const pdfUrl = await generateAndUploadPDF({ project, selectedFields, isRTL });
      const message = isRTL 
        ? `مشروع: ${project.name}\n${pdfUrl}`
  ...
  const handleDownloadPDF = async () => {
    ...
    setIsLoading(true);
    try {
      await generateAndUploadPDF({ project, selectedFields, isRTL });
      toast.success(isRTL ? 'تم تحميل الملف بنجاح' : 'File downloaded successfully');
    } catch (error) {
